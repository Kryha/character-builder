...
export default async function deployApi(homePromise, { bundleSource, pathResolve }) {
  ...
  const { creatorFacet: nftFacet } = await E(zoe).startInstance(
    installation,
  );
  ...
  console.log(`-- NFT_ISSUER_BOARD_ID: ${CARD_ISSUER_BOARD_ID}`);
  console.log(`-- NFT: ${CARD_BRAND_BOARD_ID}`);
  ...
  const dappConstants = {
    ...
    brandBoardIds: {
      Nft: CARD_BRAND_BOARD_ID,
      Money: MONEY_BRAND_BOARD_ID,
    },
    issuerBoardIds: {
      Nft: CARD_ISSUER_BOARD_ID,
